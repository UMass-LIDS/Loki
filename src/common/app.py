...
class App:
    ...
    def setThroughputsFromAllocation(self, allocationPlan: dict,
        ...
        for key in allocationPlan:
            ...
            row = executionProfiles.loc[(executionProfiles['model'].str.contains(modelVariant)) &
                                        (executionProfiles['batch_size'] == batchSize) &
                                        (executionProfiles['accelerator'] == hardware)]

            if len(row) > 0:
                latencyInSec = float(row['90th_pct_runtime'].values[0])
    ...
    def getServiceTimeForAllocation(self, allocationPlan: dict,
        ...
        for key in allocationPlan:
            ...
            if len(row) > 0:
                latencyInUSec = int(row['90th_pct_runtime'].values[0] * USecInSec)
