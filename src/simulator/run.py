...
from core.pipeline_controller import AllocationPolicy, PipelineController
...
def validate_config(config: dict, filename: str):
    model_allocation_algos = ['ilp', 'infaas', 'clipper', 'ilp', 'infaas_v2',
                              'sommelier', 'pipeline_aware', 'inferline']
    job_sched_algos = ['random', 'round_robin', 'eft_fifo', 'lft_fifo', 'infaas',
                       'canary_routing', 'most_accurate_first']
    ...
    if 'app_definitions_path' not in config:
        raise ConfigException(f'app_definitions_path not specified in config file: {filename}')
    ...
    if model_allocation in ['ilp', 'sommelier', 'pipeline_aware'] and 'beta' not in config:
    ...
    if model_allocation in ['ilp', 'sommelier', 'infaas_v2', 'pipeline_aware', 'inferline'] and 'solve_interval' not in config:
        raise ConfigException(f'solve_interval not specified for model allocation in '
                              f'config file: {filename}\nsolve_interval value is needed if '
                              f'model allocation is one of the following: ilp, sommelier, '
                              f'pipeline_aware')
    
    if not(model_allocation in ['ilp', 'sommelier', 'infaas_v2', 'pipeline_aware', 'inferline']) and 'solve_interval' in config:
        raise ConfigException(f'unexpected parameter solve_interval specificed in config: {filename}'
                              f'\solve_interval is only needed for ILP, Sommelier or '
                              f'pipeline_aware')

    if not(model_allocation in ['ilp', 'sommelier', 'pipeline_aware']) and 'beta' in config:
        raise ConfigException(f'unexpected parameter beta specificed in config: {filename}'
                              f'\nbeta is only needed for ILP, Sommelier or '
                              f'pipeline_aware')
...
def main(args):
    ...
    app_definitions_path = config['app_definitions_path']
    trace_path = config['trace_path']
    branching_trace_path = config['branching_trace_path'] if 'branching_trace_path' in config else None
    ...
    pipeline_accuracy_file = config['pipeline_accuracy_file'] if 'pipeline_accuracy_file' in config else None
    allowed_variants_path = config['allowed_variants']
    max_batch_size = config['max_batch_size'] if 'max_batch_size' in config else None
    dynamic_rerouting = config['dynamic_rerouting'] if 'dynamic_rerouting' in config else False
    no_early_dropping = config['no_early_dropping'] if 'no_early_dropping' in config else False
    last_stage_early_drop = config['last_stage_early_drop'] if 'last_stage_early_drop' in config else False

    env = SchedulingEnv(app_definitions_path=app_definitions_path, trace_dir=trace_path,
                        branching_trace_path=branching_trace_path,
                        job_sched_algo=job_scheduling, action_group_size=action_group_size,
                        ...
                        pipeline_accuracy_file=pipeline_accuracy_file,
                        dynamic_rerouting=dynamic_rerouting,
                        no_early_dropping=no_early_dropping,
                        last_stage_early_drop=last_stage_early_drop)

    # # Old way of getting rtypes before pipeline implementation
    # rtypes = len(glob.glob(os.path.join(trace_path, '*.txt')))

    # Simulator already decides number of executors, we just use that
    rtypes = env.simulator.get_num_executors()
    ...
    elif model_assignment == 'pipeline_aware':
        allocationPolicy = AllocationPolicy.ILP_COST
        pipelineController = PipelineController(simulator=env.simulator,
                                                allocationPolicy=allocationPolicy,
                                                beta=beta)
        print('Testing with Pipeline-Aware model allocation')
    elif model_assignment == 'inferline':
        allocationPolicy = AllocationPolicy.INFERLINE
        pipelineController = PipelineController(simulator=env.simulator,
                                                allocationPolicy=allocationPolicy)
        print('Testing with InferLine model allocation')
    else:
        print(f'Undefined model assignment: {model_assignment}, exiting')
        sys.exit(0)
    print('--------------------------')
    print()
    ...
    for i in range(testing_steps):
        # Adding EWMA demand to the simulator
        ...
        if model_assignment == 'ilp' or model_assignment == 'sommelier':
            ...
            if ilp_applied == True:
                ...
                if i % period_tuning == 0 or i == 5 or i == 20:
                    actions = ilp.run(observation, env.n_accelerators,
                                      env.max_no_of_accelerators)
                    ilp_rounds += 1
            ...
            action = env.simulator.null_action(env.action_space.sample(), 1)
        elif model_assignment == 'pipeline_aware' or model_assignment == 'inferline':
            if i % solve_interval == 0:
                pipelineController.eventLoop(observation=observation,
                                             num_acc_types=env.n_accelerators,
                                             num_max_acc=env.max_no_of_accelerators)
            action = env.simulator.null_action(env.action_space.sample(), 1)
        else:
        ...
        if max(available_predictors) > env.max_no_of_accelerators or min(available_predictors) < 0:
            raise Exception(f'Invalid available predictors state: '
    ...
    bumped_failed_per_model = list(env.simulator.slo_timeouts_per_executor['timeouts'].values())
    ...
    print()
    pending_removal_preds = env.simulator.get_pending_removal_predictors()
    print(f'Pending-removal predictors (length {len(pending_removal_preds)}): '
          f'{pending_removal_preds}')
    ...
    print(f'Total parent requests: {env.simulator.parent_requests_added}')
    print(f'Total requests: {bumped_total}')
    print(f'Dropped requests: {bumped_failed}, ({bumped_failed_per_model} '
          f'dropped recorded in per_model stats), dropped ratio: '
    ...
    request_times_file = open('logs/request_times.csv', mode='w')
    request_times_file.write('request_id,waiting_time,execution_time,model_family\n')
    for request_id in env.simulator.request_times:
        waiting_time = None
        execution_time = None
        model_family = None
        if 'waiting_time' in env.simulator.request_times[request_id]:
            waiting_time = env.simulator.request_times[request_id]['waiting_time']
        if 'execution_time' in env.simulator.request_times[request_id]:
            execution_time = env.simulator.request_times[request_id]['execution_time']
        if 'model_family' in env.simulator.request_times[request_id]:
            model_family = env.simulator.request_times[request_id]['model_family']
        request_times_file.write(f'{request_id},{waiting_time},{execution_time},'
                                 f'{model_family}\n')
