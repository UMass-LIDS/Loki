...
class Executor:
    def __init__(self, isi, task_assignment, logging_level, n_qos_levels=1,
                 ...
                 configured_max_batch_size=None, application=None):
        ...
        self.predictors = {}
        self.pending_removal_predictors = {}
        self.ready_to_remove_predictors = []
        ...
        # Which application the executor belongs to
        self.application = application
        self.log.info(f'Registered executor {isi} for application: '
                      f'{application.getName()}')
    ...
    def add_predictor(self, acc_type=AccType.GPU, qos_level=0, variant_name=None):
        ...
        self.log.info(f'Load balancer triggered by executor.add_predictor')
        self.simulator.loadBalancer.balanceLoad()

        if any(num < 0 for num in self.simulator.available_predictors):
            raise Exception(f'simulator available_predictors: {self.simulator.available_predictors}')

        return predictor.id
    
    
    def predictor_ready_to_remove(self, predictor: Predictor):
        if predictor.id in self.pending_removal_predictors:
            self.log.info(f'Actually removed pending-removal predictor: '
                          f'{predictor.id}')
            # print(f'Actually removed pending-removal predictor: {predictor.id}')
            self.ready_to_remove_predictors.append(predictor.id)
        else:
            raise Exception(f'predictor ({predictor.id}) not in pending removal'
                            f' predictors: {self.pending_removal_predictors}')
        
    
    def remove_ready_to_remove_predictors(self):
        ''' Remove predictors that are in ready_to_remove state (from pending 
            predictors list) and flush ready_to_remove list
            
        '''
        for predictorId in self.ready_to_remove_predictors:
            del self.pending_removal_predictors[predictorId]
        self.ready_to_remove_predictors = []
    ...
    def remove_predictor_by_id(self, id):
        if id in self.predictors:
            ...
            predictor = self.predictors[id]
            del self.predictors[id]
            self.iterator = itertools.cycle(self.predictors)

            self.log.warning(f'We are indicating that new predictors can be '
                             f'added in place of pending-removal predictors, '
                             f'which is wrong. We are adding more predictors '
                             f'than the system supports')
            self.pending_removal_predictors[id] = predictor
            predictor.set_pending_removal()

            self.log.info(f'Load balancer triggered by executor.remove_'
                          f'predictor_by_id (removing {id})')
            self.simulator.loadBalancer.balanceLoad()
            return True
        else:
            return False
    ...
    def remove_predictor_by_type(self, acc_type, qos_level=0):
        ...
        for id in self.predictors:
            ...
            if acc_type == predictor_type and qos_level == predictor_qos:
                ...
                self.log.info(f'Load balancer triggered by executor.remove_'
                            f'predictor_by_type (removing {id})')
                self.simulator.loadBalancer.balanceLoad()
                return True
        return False
    ...
    def process_request(self, event, clock, runtimes):
        if len(self.predictors) == 0:
            foundNewAccType = False
            for i in range(len(self.simulator.available_predictors)):
                if self.simulator.available_predictors[i] > 0:
                    acc_type = i+1
                    foundNewAccType = True
                    break
            # If no accelerator type has available predictors, cluster
            # is full and we have to fail the request
            if not(foundNewAccType):
                self.simulator.bump_failed_request_stats(event, clock)
                return

            added = self.add_predictor(acc_type=AccType(acc_type))
            if added is not None:
                added_predictor = self.predictors[added]
                acc_type = added_predictor.acc_type
                self.simulator.available_predictors[acc_type-1] -= 1
                print(f'added predictor from executor (variant: '
                      f'{added_predictor.variant_name}, acc_type: {acc_type}, id: '
                      f'{self.id}), simulator.available_predictors: '
                      f'{self.simulator.available_predictors}')
                if any(num < 0 for num in self.simulator.available_predictors):
                    raise Exception(f'Unexpected simulator state. Predictors: '
                                    f'{self.simulator.available_predictors}')
            else:
                raise Exception('Predictor could be not be added')
    ...
    def trigger_infaas_v2_upscaling(self):
        ...
        for id in predictors_to_remove:
            ...
            if removed:
                ...
                max_acc_per_type = self.simulator.max_acc_per_type
                if any(num > max_acc_per_type for num in self.simulator.available_predictors):
                    raise Exception(f'Unexpected simulator state. Predictors: '
                                    f'{self.simulator.available_predictors}')
    ...
    def finish_request(self, event, clock):
        if event.id not in self.assigned_requests:
            self.log.error('Could not find assigned request.')
            time.sleep(1)
            return False
        
        predictor = self.assigned_requests[event.id]

        finished = predictor.finish_request(event, clock)
        if finished:
    ...
    def enqueue_request(self, event, clock, target_predictor_id=None):
        ...
        if target_predictor_id is not None:
            predictor_assigned = self.enqueue_request_to_predictor(event,
                                                                   clock,
                                                                   target_predictor_id)
            # Corner case: predictor was deleted while request was enroute to
            # it, reroute it to a different predictor
            if predictor_assigned is not None:
                self.log.debug(f'\nExecutor {self.isi} assigned request to target '
                            f'predictor id: {predictor_assigned.id}, task: '
                            f'{predictor_assigned.executor.isi}\n')
        elif self.task_assignment == TaskAssignment.CANARY:
            predictor_assigned = self.enqueue_request_canary(event, clock)
            self.log.debug(f'\nExecutor {self.isi} assigned request through canary '
                           f'routing to predictor: {predictor_assigned}\n')
        elif self.task_assignment == TaskAssignment.INFAAS:
            predictor_assigned = self.enqueue_request_infaas(event, clock)
            self.log.debug(f'\nExecutor {self.isi} assigned request using INFaaS '
                           f'policy to predictor: {predictor_assigned}\n')
        elif self.task_assignment == TaskAssignment.MOST_ACCURATE_FIRST:
            predictor_assigned = self.enqueue_request_maf(event, clock)
            self.log.debug(f'\nExecutor {self.isi} assigned request using Most-'
                           f'Accurate-First routing policy to predictor: '
                           f'{predictor_assigned}\n')
        else:
            raise ExecutorException(f'Unrecognized task assignment/job scheduling '
                                    f'algorithm for batch request processing: '
                                    f'{self.task_assignment}')
        
        if predictor_assigned is None:
            self.simulator.bump_failed_request_stats(event, clock)
        else:
            self.assigned_requests[event.id] = predictor_assigned

        return
        
    
    def enqueue_request_to_predictor(self, event, clock, predictor_id):
        if predictor_id not in self.predictors:
            # This is the case where predictor was selected but it has been
            # removed before request could be assigned to it
            # How could this happen in the real system?
            # 1. In the real system, this could happen due to a delay in the
            #    update of routing tables. Perhaps routing table update should
            #    be trigerred everytime there is a change (if protocol in 3 is
            #    followed, this should not happen)
            # 2. A request could already be in-network when the predictor is
            #    removed (but if the protocol in 3 is followed, this should
            #    not happen)
            # 3. A predictor should finish its queue before being physically
            #    removed
            #    The order is this: Controller decides to remove predictor ->
            #    Load balancer updates routing tables so that new requests are
            #    not sent -> Verification step here to make sure predictor gets
            #    no new requests (if it does, raise an alarm) -> Predictor 
            #    finishes its old queue before being physically removed ->
            #    New predictor is added and ready to serve -> Load balancer
            #    updates routing tables
            #  
            #    KEY TAKEAWAY: Routing table update should be triggered whenever
            #    predictors are added or removed from the system

            # Logical problem in algorithm: When a predictor is pending removal,
            # it is removed from routing tables of others and it does not get any
            # routing updates to its table either. However, it is still processing
            # outstanding requests, and since it does not get any routing updates,
            # it will be sending to predictors which are deleted after it
            # (Verified by logs)
            # The logical solution is to keep sending it new updates but do not
            # include it in the list of predictors to whom requests can be sent
            # (it can send but not receive)
            # Eventually it would be removed by the system from the list of
            # pending-removal predictors and it would not get any updates either

            # TODO: If this happens, is this an error?
            #       If so, where did the logical problem originate? (explanation
            #       above)
            self.log.error(f'enqueue_request_to_predictor(): target predictor '
                           f'no longer exists (id: {predictor_id}), dropping '
                           f'request. self.predictors: '
                           f'{list(map(lambda x: x[1].id, self.predictors.items()))}')
            return None
    ...
    def enqueue_request_maf(self, event, clock):
        ''' Enqueues request to appropriate predictor according Most-Accurate-
            First load balancing/request routing policy
        '''
        # Get predictors for the executor
        predictors = list(map(lambda x: self.predictors[x], self.predictors))

        # Sort predictor in non-increasing order of single-model accuracy
        sorted_predictors = sorted(predictors, key=lambda x: x.profiled_accuracy,
                                   reverse=True)
        
        # Find the first predictor that has enough capacity to enqueue a new
        # request
        selected_predictor = None
        overload_dict = {}
        for predictor in sorted_predictors:
            # If the current QPS is more than the peak throughput of the
            # predictor, it is overloaded
            queue_length = len(predictor.request_queue)
            if queue_length == 0:
                selected_predictor = predictor
                break

            msec_since_first_request = clock - predictor.request_queue[0].start_time
            sec_since_first_request = msec_since_first_request / MSECS_IN_SEC

            # Corner case, the first request in the queue was just added, which
            # means there is enough capacity at this predictor
            if sec_since_first_request == 0:
                selected_predictor = predictor
                break

            current_qps = queue_length / sec_since_first_request
            if current_qps > predictor.peak_throughput:
                overload = current_qps / predictor.peak_throughput
                overload_dict[predictor] = overload
                continue
            else:
                selected_predictor = predictor
                break

        # If all predictors are overloaded, no predictor will be selected
        # by the algorithm above
        # In this case, send the request to the least overloaded predictor
        if selected_predictor is None:
            min_overload = sys.maxsize
            for predictor in overload_dict:
                if overload_dict[predictor] < min_overload:
                    min_overload = overload_dict[predictor]
                    selected_predictor = predictor

        # If still no predictor could be found, it means there are no existing
        # predictors for this executor
        if selected_predictor is None:
            raise Exception(f'No predictor could be found for executor: '
                            f'{self.isi}')
        
        # Enqueue the request at the selected predictor
        selected_predictor.enqueue_request(event, clock)

        return selected_predictor
        
    
    def enqueue_request_canary(self, event, clock):
        ''' Enqueues request to appropriate predictor acccording to canary
            routing load balancing policy.
        '''
    ...
    def enqueue_request_infaas(self, event, clock):
